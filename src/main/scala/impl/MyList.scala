...
sealed abstract class MyList[+A] {
  ...
  final def map3[B](f: A => B): MyList[B] = foldRight(MyList[B]()) {
    (xs, acc) =>
      f(xs) :: acc
  }

  final def flatMap2[B](f: A => MyList[B]): MyList[B] =
    foldLeft[MyList[B]](MyNil) { (acc, xs) =>
      acc ::: f(xs)
    }.reverse

  final def filter2(f: A => Boolean): MyList[A] =
    foldLeft[MyList[A]](MyNil) { (acc, xs) =>
      if (f(xs)) xs :: acc else acc
    }.reverse

  final def foldRight2[B](z: B)(f: (A, B) => B): B =
    foldLeft((x: B) => x) { (acc, xs) => x =>
      acc(f(xs, x))
    }(z)
...
case object MyNil extends MyList[Nothing] {
  ...
  override def tail: MyList[Nothing] =
    throw new UnsupportedOperationException("tail of empty MyList")
}
